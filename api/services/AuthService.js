  ...
  authenticate: function (req, res, permission_required, callback) {
    ...
    Tokens.findOne({
      ...
      if (permission_required == "admin") {
        if (token_object.users.usertype != "admin") return RespService.e(res, 'Only admins can use this function');
      ...
      if (!token_object.supertoken) {
        ...
        if (!token_object.permissions) return RespService.e(res, 'Something is really broken');
        if (token_object.permissions.name != permission_required) return RespService.e(res, 'Wrong permission type for this token');
      }
